<template>
  <div class="page-root">
    ...
    <div class="page-header">
      <div>
        <h1 class="page-title">Pending Vendors</h1>
        <p class="page-sub">Review and manage vendors awaiting approval</p>
      </div>
      <div class="header-actions">
        <button class="icon-btn" @click="refresh" :class="{ spinning: loading }">
          <span class="mdi mdi-refresh"></span>
        </button>
        <button class="btn-add" @click="router.push('/aggregator/vendor_onboarding')">
          ...
          <tbody v-else>
            ...
            >
              ...
              <td @click.stop>
                <button
                  class="btn-view"
                  :class="pendingActionClass(item.mstatus)"
                  @click="goToVendor(item)"
                  :title="actionTitle(item.mstatus)"
                >
                  <span class="mdi" :class="actionIcon(item.mstatus)"></span>
                </button>
              </td>
            </tr>
            ...
            <tr v-if="filteredList.length === 0">
              <td colspan="8" class="empty-td">
                <div class="empty-state">
                  ...
                  <button class="btn-add btn-add-sm" @click="router.push('/aggregator/vendor_onboarding')">
                    <span class="mdi mdi-plus"></span>
                    Add New Vendor
                  </button>
                </div>
...
const pendingActionClass = (s: string) => ({
  'btn-view-amber': s === 'PENDING' || !s,
  'btn-view-blue':  s === 'ONBOARDED',
});
const actionIcon  = (s: string) => s === 'PENDING' || !s ? 'mdi-account-clock-outline' : s === 'ONBOARDED' ? 'mdi-arrow-right-circle-outline' : 'mdi-eye-outline';
const actionTitle = (s: string) => s === 'PENDING' || !s ? 'Continue Onboarding' : s === 'ONBOARDED' ? 'View Onboarding' : 'View Vendor';
...
const statsStrip = computed(() => {
  ...
  return [
    { label: 'Total',      value: store.totalKycPending,                                       color: '#1142d4' },
    { label: 'Pending',    value: all.filter((v:any) => v.mstatus === 'PENDING' || !v.mstatus).length, color: '#f59e0b' },
    { label: 'Onboarded',  value: all.filter((v:any) => v.mstatus === 'ONBOARDED').length,     color: '#7c3aed' },
    { label: 'Approved',   value: all.filter((v:any) => v.mstatus === 'APPROVED').length,      color: '#22c55e' },
  ];
});
...
async function load() {
  loading.value = true;
  try {
    const res = await getPendingVendors({ page: kycPendingPage.value, limit: kycPendingLimit.value });
    store.setKycPending({
...
const onKycPageChange  = (p: number) => { kycPendingPage.value = p; load(); };
const onKycLimitChange = (l: number) => { kycPendingLimit.value = l; kycPendingPage.value = 1; load(); };

/* ── Row click ── */
const goToVendor = (item: any) => router.push(`/aggregator/vendor_onboarding/${item.id}`);
...
.btn-view-blue         { background: rgba(17,66,212,.07); border-color: rgba(17,66,212,.2); color: #1142d4; }
.btn-view-blue:hover   { background: #1142d4; color: #fff; border-color: #1142d4; }
