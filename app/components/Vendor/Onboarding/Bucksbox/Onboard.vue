...
// Final submit is BUCKSBOX-specific (no real external gateway to call) —
// marks the merchant's UPI/BUCKSBOX MerchantServiceKyc row SUBMITTED
// directly, rather than routing through ISG's shared /submit/onboading.
const { markBucksboxSubmitted } = useBucksboxApi();

// This page reuses ISG's onboarding routes/composable verbatim (same OTP,
// PAN, bank-account verification and document-compliance flow) — every
// request below just tags itself as BUCKSBOX so KYC reads/writes land on
// the merchant's UPI/BUCKSBOX row instead of UPI/ISG. See isg.route.js's
// resolveInterface() on the backend for the other half of this.
const KYC_INTERFACE = "BUCKSBOX";
...
async function handleStep2VerifyOtp() {
  ...
  try {
    const res = await isgVerifyOtp({ phone: otpPhone.value, otp: otpCode.value, interface: KYC_INTERFACE });
...
async function handleStep3Pan() {
  ...
  try {
    ...
    console.error("[BUCKSBOX] PAN error:", err);
  } finally {
    loading.value = false;
  }
}
...
async function handleStep4Account() {
  ...
  try {
    ...
    console.error("[BUCKSBOX] Account error:", err);
  } finally {
    loading.value = false;
  }
}
...
async function loadComplianceAndProceed() {
  ...
  try {
    ...
    const status = await complianceStatus(props.merchantId, KYC_INTERFACE);
    ...
    console.error("[BUCKSBOX] Compliance status error:", err);
  } finally {
...
async function saveAadhaarDoc() {
  ...
  try {
    ...
    const res = await complianceInit(payload, props.merchantId, KYC_INTERFACE);
    if (res.data?.statusCode === "00" || res.statusCode === "00") {
      const status = await complianceStatus(props.merchantId, KYC_INTERFACE);
...
async function saveDocUpload() {
  ...
  try {
    ...
    console.error("[BUCKSBOX] Doc upload error:", err);
  } finally {
    savingDoc.value = false;
  }
}
...
// ── Step 6 — Submit BUCKSBOX Onboarding ───────────────────────────────────────────
async function handleStep6Submit() {
  ...
  try {
    ...
    if (res?.statusCode === "00") {
      ...
      showSnack(res?.message || "Onboarding submitted successfully!", "success");
      step.value = 7;
    } else {
      showSnack(res?.message || "Submission failed. Please retry.", "error");
    }
  } catch (err) {
    showSnack("An error occurred during submission.", "error");
    console.error("[BUCKSBOX] Submit error:", err);
  } finally {
