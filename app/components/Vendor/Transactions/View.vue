  ...
  <div class="view-root">
    ...
    <template v-else-if="txn">
      ...
      <div class="info-grid">
        ...
        <!-- Merchant Info -->
        <div class="info-card">
          <div class="card-hdr"><span class="mdi mdi-store-outline card-hdr-icon" style="color:#059669"></span>Merchant</div>
          <div class="field-list">
            <div class="field-row"><span class="field-lbl">Name</span><span class="field-val">{{ txn.merchant?.legal_name ?? '—' }}</span></div>
            <div class="field-row"><span class="field-lbl">Business</span><span class="field-val">{{ txn.merchant?.business_name ?? '—' }}</span></div>
            <div class="field-row"><span class="field-lbl">MID</span><span class="field-val mono">{{ txn.merchant?.mid ?? '—' }}</span></div>
            <div class="field-row"><span class="field-lbl">KYC Status</span><span class="field-val">{{ txn.merchant?.mstatus ?? '—' }}</span></div>
            <div class="field-row"><span class="field-lbl">Status</span><span class="field-val">{{ txn.merchant?.status ?? '—' }}</span></div>
          </div>
        </div>

        <!-- Terminal -->
        <div class="info-card">
          <div class="card-hdr"><span class="mdi mdi-devices card-hdr-icon" style="color:#7c3aed"></span>Terminal & Vendor</div>
          <div class="field-list">
...
const { getVendorTransactionById } = useVendorApi();
...
const load = async () => {
  ...
  try {
    ...
    const res = await getVendorTransactionById(id);
