    ...
    <main class="mh-content">
                      ...
                      <div class="doc-verify-cards doc-verify-cards--mobile">
                        <div class="doc-verify-card" v-for="doc in [
                          ...
                          <div class="doc-verify-card__meta" v-if="doc.verifiedAt || doc.reason">
                            <span v-if="doc.verifiedAt" class="doc-verify-card__date">✓ {{ formatDate(doc.verifiedAt)
                              }}</span>
      ...
      <section v-show="activeTab === 'commission'" class="tab-section">
        ...
        <div class="card">
          <div class="card__head">
            <div class="card__head-dot card__head-dot--indigo"></div>
            <h3 class="card__title">Merchant Commission Configuration</h3>
          ...
          <div v-if="merchantForm.commissionconfig?.length" class="table-scroll-wrap">
            <table class="data-table">
              <thead>
                <tr>
                  <th>Method</th>
              ...
              <tbody>
                <tr v-for="cfg in merchantForm.commissionconfig" :key="cfg.id">
                  <td><span class="pill pill--sky">{{ cfg.paymentMethod }}</span></td>
                  <td>{{ cfg.provider }}</td>
                  <td>{{ cfg.txnType }}</td>
                  <td class="font-mono">₹ {{ Number(cfg.minAmount).toLocaleString('en-IN') }}</td>
                  <td class="font-mono">₹ {{ Number(cfg.maxAmount).toLocaleString('en-IN') }}</td>
                  <td class="font-bold text-indigo-600">
                    {{ cfg.merchantRate }}{{ cfg.merchantRateType === 'PERCENTAGE' ? '%' : ' ₹' }}
                  </td>
                  <td class="font-bold text-violet-600">
                    {{ cfg.vendorRate }}{{ cfg.vendorRateType === 'PERCENTAGE' ? '%' : ' ₹' }}
                  </td>
                  <td class="font-bold text-sky-600">
                    {{ cfg.aggregatorRate }}{{ cfg.aggregatorRateType === 'PERCENTAGE' ? '%' : ' ₹' }}
                  </td>
                  <td class="font-bold text-rose-600">
                    {{ cfg.bankRate }}{{ cfg.bankRateType === 'PERCENTAGE' ? '%' : ' ₹' }}
                  </td>
                  <td>
                    <span :class="['pill', cfg.active ? 'pill--emerald' : 'pill--red']">
                      {{ cfg.active ? 'Yes' : 'No' }}
                    </span>
                  </td>
                  <td>
                    <div class="action-btns">
                      <button class="icon-btn icon-btn--edit" @click="openEditCommission(cfg)" title="Edit">
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                      ...
                      <button class="icon-btn icon-btn--delete" @click="deleteCommissionCfg(cfg.id)" title="Delete">
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                          stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
          ...
          <div v-else class="empty-state">
            ...
            <p>No commission config set for this merchant</p>
            <button class="btn-primary" style="margin-top:10px;" @click="openAddCommission">Add First
              Commission</button>
          </div>
        </div>
              ...
              <div class="modal-box">
                <div class="modal-box__header">
                  <h3 class="modal-box__title">
                    {{ commissionModal.mode === 'add' ? 'Add Commission Config' : 'Edit Commission Config' }}
                  </h3>
                    ...
                    <div class="modal-field modal-field--full">
                      <label class="toggle-label">
                        ...
                        <div class="toggle-wrap">
                          <input type="checkbox" v-model="commissionForm.active" class="toggle-input"
                            id="activeToggle" />
                          <label for="activeToggle" class="toggle-track"></label>
                        </div>
                      </label>
                    </div>
                ...
                <div class="modal-box__footer">
                  ...
                  <button class="btn-primary" :disabled="commissionSaving" @click="saveCommission">
                    <svg v-if="commissionSaving" class="spin-icon" width="14" height="14" viewBox="0 0 24 24"
                      fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"
                      stroke-linejoin="round">
                      <path d="M21 12a9 9 0 1 1-6.219-8.56" />
                    </svg>
                    {{ commissionSaving ? 'Saving…' : commissionModal.mode === 'add' ? 'Add Config' : 'Update Config' }}
                  </button>
                </div>
              </div>
            </div>
        ...
        <div v-if="mobileDrawerOpen" class="mobile-drawer">
          ...
          <div class="mobile-drawer__body">
            ...
            <template v-if="!mobileDrawerStep2">
              ...
              <div v-for="svc in servicesOptions" :key="svc.id" class="mobile-drawer__item"
                @click="mobileDrawerStep2 = svc">
            ...
            <template v-else>
              ...
              <div v-for="intf in mobileDrawerStep2.interfaces" :key="intf.id" class="mobile-drawer__item"
                @click="mobileConfirmOnboard(intf)">
                <div class="mobile-drawer__item-icon mobile-drawer__item-icon--violet">{{ intf.interface.slice(0, 2) }}
                </div>
                <div class="mobile-drawer__item-info">
...
const onboardService = async (svc, intf, kycStatus) => {
  try {
    ...
    } else if (svc === "DMT" && intf === "NSDL") {
      router.push({
        path: `/vendor/onboarding/dmt/${props.merchantId}`,
        query: {
          pan_status: kycStatus?.pan_status || "PENDING",
          otp_status: kycStatus?.otp_status || false,
          aadhaar_status: kycStatus?.aadhaar_status || "PENDING",
...
// ── Commission state ──────────────────────────────────────────────────
import { useCommissionApi } from '~/composables/apis/useCommissionApi'
const { createCommissionConfig, updateCommissionConfig, deleteCommissionConfig } = useCommissionApi()
...
const defaultCommissionForm = () => ({
  ...
  maxAmount: 99999999,
  merchantRate: 0,
  merchantRateType: 'PERCENTAGE',
  vendorRate: 0,
  vendorRateType: 'PERCENTAGE',
  aggregatorRate: 0,
  aggregatorRateType: 'PERCENTAGE',
  bankRate: 0,
  bankRateType: 'PERCENTAGE',
  active: true,
})
...
const openEditCommission = (cfg) => {
  Object.assign(commissionForm, {
    ...
    merchantRate: cfg.merchantRate,
    merchantRateType: cfg.merchantRateType,
    vendorRate: cfg.vendorRate,
    vendorRateType: cfg.vendorRateType,
    aggregatorRate: cfg.aggregatorRate,
    aggregatorRateType: cfg.aggregatorRateType,
    bankRate: cfg.bankRate,
    bankRateType: cfg.bankRateType,
    active: cfg.active,
  })
...
const saveCommission = async () => {
  if (!commissionForm.paymentMethod || !commissionForm.provider) {
    snackbar.message = 'Payment Method and Provider are required'
    snackbar.color = 'error'
    snackbar.show = true
    setTimeout(() => { snackbar.show = false }, 3000)
    return
  ...
  try {
    const payload = {
      ...
      merchantRate:      commissionForm.merchantRate,
      merchantRateType:  commissionForm.merchantRateType,
      vendorRate:        commissionForm.vendorRate,
      vendorRateType:    commissionForm.vendorRateType,
      aggregatorRate:    commissionForm.aggregatorRate,
      aggregatorRateType: commissionForm.aggregatorRateType,
      bankRate:          commissionForm.bankRate,
      bankRateType:      commissionForm.bankRateType,
      active:            commissionForm.active,
    }

    let res
    if (commissionModal.mode === 'add') {
      res = await createCommissionConfig(props.merchantId, payload)
    } else {
      res = await updateCommissionConfig(props.merchantId, commissionForm.id, payload)
    }

    const ok = res?.statusCode === '00'
    snackbar.message = ok ? (res.message || 'Saved successfully') : (res?.message || 'Something went wrong')
    snackbar.color   = ok ? 'success' : 'error'
    snackbar.show    = true

    if (ok) {
      closeCommissionModal()
      setTimeout(() => getMerchant(props.merchantId), 800)
    }
  } catch (e) {
    snackbar.message = 'Failed to save commission config'
    snackbar.color   = 'error'
    snackbar.show    = true
  } finally {
...
const deleteCommissionCfg = async (cfgId) => {
  if (!confirm('Delete this commission config?')) return
  try {
    const res = await deleteCommissionConfig(props.merchantId, cfgId)
    const ok  = res?.statusCode === '00'
    snackbar.message = ok ? 'Deleted successfully' : (res?.message || 'Delete failed')
    snackbar.color   = ok ? 'success' : 'error'
    snackbar.show    = true
    if (ok) setTimeout(() => getMerchant(props.merchantId), 800)
  } catch {
...
.doc-verify-card__date {
  font-size: 10.5px;
  color: #6b7280;
}

.doc-verify-card__reason {
  font-size: 10.5px;
  color: #ef4444;
}
...
.mobile-drawer__item-icon--violet {
  background: #6d28d9;
}

.mobile-drawer__item-info {
  flex: 1;
  min-width: 0;
}
...
@media (max-width: 640px) {
  ...
  /* ── Limit rows ── */
  .limit-row {
    grid-template-columns: 60px 1fr 72px 52px;
    gap: 6px;
  }
...
.modal-field select:focus { border-color: #6366f1; box-shadow: 0 0 0 3px rgba(99,102,241,.1); }
