  ...
  <div class="vendor-shell">
        ...
        <div class="mh-header__brand">
          <div class="mh-brand-text">
            ...
            <div class="mh-meta mh-meta--right">
              ...
              <span :class="['mh-badge', vendorForm.status ? 'mh-badge--success' : 'mh-badge--error']">{{
                vendorForm.status ? 'Active' : 'Inactive' }}</span>
            </div>
          </div>
          <div class="mh-logo">
    ...
    <main class="mh-content">
      ...
      <section v-show="activeTab === 'info'" class="tab-section">
        ...
        <div class="card">
          <div class="card__head">
            ...
            <button class="edit-toggle-btn ml-auto" @click="toggleEdit('contact')">
              <svg v-if="!editMode.contact" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor"
          ...
          <template v-if="!editMode.contact">
            <div class="info-grid info-grid--4">
              <div class="info-item"><label>Legal Name</label>
                <p>{{ vendorForm.name || '—' }}</p>
              </div>
              <div class="info-item"><label>Email</label>
                <p>{{ vendorForm.email || '—' }}</p>
              </div>
              <div class="info-item"><label>Mobile</label>
                <p>{{ vendorForm.mobile_no || '—' }}</p>
              </div>
              <div class="info-item"><label>Vendor Code</label>
                <p class="font-mono">{{ vendorForm.code || '—' }}</p>
              </div>
              <div class="info-item"><label>Status</label>
                <p><span :class="['pill', vendorForm.status ? 'pill--emerald' : 'pill--red']">{{ vendorForm.status ?
                  'Active' : 'Inactive' }}</span></p>
              </div>
              <div class="info-item"><label>M-Status</label>
                <p><span class="pill pill--amber">{{ vendorForm.mstatus || '—' }}</span></p>
              </div>
              <div class="info-item"><label>Risk Flag</label>
                <p><span :class="['pill', vendorForm.riskflag > 0 ? 'pill--red' : 'pill--emerald']">{{
                  vendorForm.riskflag }}</span></p>
              </div>
              <div class="info-item"><label>Latitude</label>
                <p class="font-mono">{{ vendorForm.lat || '—' }}</p>
              </div>
              <div class="info-item"><label>Longitude</label>
                <p class="font-mono">{{ vendorForm.long || '—' }}</p>
              </div>
              <div class="info-item"><label>Created At</label>
                <p>{{ formatDate(vendorForm.createdAt) }}</p>
              </div>
              <div class="info-item"><label>Updated At</label>
                <p>{{ formatDate(vendorForm.updatedAt) }}</p>
              </div>
            </div>
          </template>
          <template v-else>
        ...
        <div class="card">
          <div class="card__head">
            ...
            <button class="edit-toggle-btn ml-auto" @click="toggleEdit('settlement')"
              v-if="vendorForm.settlementAccount">
              <svg v-if="!editMode.settlement" width="12" height="12" viewBox="0 0 24 24" fill="none"
          ...
          <template v-if="vendorForm.settlementAccount">
            <template v-if="!editMode.settlement">
              <div class="info-grid info-grid--4">
                <div class="info-item"><label>Account Holder</label>
                  <p>{{ vendorForm.settlementAccount.account_holder_name }}</p>
                </div>
                <div class="info-item"><label>Account Type</label>
                  <p>{{ vendorForm.settlementAccount.account_type }}</p>
                </div>
                <div class="info-item"><label>Bank Name</label>
                  <p>{{ vendorForm.settlementAccount.bank_name }}</p>
                </div>
                <div class="info-item"><label>Account No</label>
                  <p class="font-mono">{{ vendorForm.settlementAccount.bank_account_no }}</p>
                </div>
                <div class="info-item"><label>IFSC Code</label>
                  <p class="font-mono">{{ vendorForm.settlementAccount.bank_ifsc_code }}</p>
                </div>
                <div class="info-item"><label>City</label>
                  <p>{{ vendorForm.settlementAccount.city || '—' }}</p>
                </div>
                <div class="info-item"><label>State</label>
                  <p>{{ vendorForm.settlementAccount.state || '—' }}</p>
                </div>
              </div>
            </template>
            <template v-else>
              <div class="edit-form-grid">
                <div class="edit-field"><label>Account Holder</label><input
                    v-model="vendorForm.settlementAccount.account_holder_name" /></div>
                <div class="edit-field"><label>Account Type</label><input
                    v-model="vendorForm.settlementAccount.account_type" /></div>
                <div class="edit-field"><label>Bank Name</label><input
                    v-model="vendorForm.settlementAccount.bank_name" /></div>
                <div class="edit-field"><label>Account No</label><input
                    v-model="vendorForm.settlementAccount.bank_account_no" /></div>
                <div class="edit-field"><label>IFSC Code</label><input
                    v-model="vendorForm.settlementAccount.bank_ifsc_code" /></div>
          ...
          <div class="empty-state" v-else><svg width="36" height="36" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
              <rect x="2" y="5" width="20" height="14" rx="2" />
              <line x1="2" x2="22" y1="10" y2="10" />
            </svg>
            <p>No settlement account configured</p>
          </div>
        </div>
        ...
        <div class="card" v-if="vendorForm.address">
          <div class="card__head">
            ...
            <button class="edit-toggle-btn ml-auto" @click="toggleEdit('address')">
              <svg v-if="!editMode.address" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
              </svg>
            ...
            <div class="addr-grid">
              <div class="addr-card">
                ...
                <div class="addr-card__body">
                  ...
                  <p class="addr-city">{{ vendorForm.address.city }}, {{ vendorForm.address.state }} – {{
                    vendorForm.address.pincode }}</p>
                  <div class="addr-contact">
                    <span>📞 {{ vendorForm.address.mobile_number }}</span>
                    <a v-if="vendorForm.address.website" :href="vendorForm.address.website" class="addr-link"
              ...
              <div class="addr-card">
                ...
                <div class="addr-card__body">
                  ...
                  <p class="addr-city">{{ vendorForm.address.res_city }}, {{ vendorForm.address.res_state }} – {{
                    vendorForm.address.res_pincode }}</p>
              ...
              <div class="addr-card">
                ...
                <div class="addr-card__body">
                  ...
                  <p class="addr-city">{{ vendorForm.address.v_city }}, {{ vendorForm.address.v_state }} – {{
                    vendorForm.address.v_pincode }}</p>
          ...
          <template v-else>
            <div class="edit-form-grid edit-form-grid--padded">
              <div class="edit-field"><label>Official Address</label><input
                  v-model="vendorForm.address.official_address" /></div>
        ...
        <div class="card" v-if="vendorForm.user">
          ...
          <div class="info-grid info-grid--4">
            <div class="info-item"><label>Name</label>
              <p>{{ vendorForm.user.name }}</p>
            </div>
            <div class="info-item"><label>Email</label>
              <p>{{ vendorForm.user.email }}</p>
            </div>
            <div class="info-item"><label>Mobile</label>
              <p>{{ vendorForm.user.mobile_no }}</p>
            </div>
            <div class="info-item"><label>Role</label>
              <p><span class="pill pill--indigo">{{ vendorForm.user.role }}</span></p>
            </div>
            <div class="info-item"><label>Account Status</label>
              <p><span :class="['pill', vendorForm.user.status ? 'pill--emerald' : 'pill--red']">{{
                vendorForm.user.status ?
                  'Active' : 'Inactive' }}</span></p>
            </div>
            <div class="info-item"><label>Mobile Verified</label>
              <p><span :class="['pill', vendorForm.user.mobile_verifited ? 'pill--emerald' : 'pill--amber']">{{
                vendorForm.user.mobile_verifited ? 'Yes' : 'No' }}</span></p>
            </div>
            <div class="info-item"><label>Email Verified</label>
              <p><span :class="['pill', vendorForm.user.isemailVerified ? 'pill--emerald' : 'pill--amber']">{{
                vendorForm.user.isemailVerified ? 'Yes' : 'No' }}</span></p>
            </div>
            <div class="info-item"><label>OTP Verified</label>
              <p><span :class="['pill', vendorForm.user.isotpVerified ? 'pill--emerald' : 'pill--amber']">{{
                vendorForm.user.isotpVerified ? 'Yes' : 'No' }}</span></p>
            </div>
            <div class="info-item"><label>Invited</label>
              <p><span :class="['pill', vendorForm.user.invited ? 'pill--emerald' : 'pill--amber']">{{
                vendorForm.user.invited ?
                  'Yes' : 'Pending' }}</span></p>
            </div>
            <div class="info-item"><label>Joined</label>
            ...
            <table class="data-table">
              ...
              <tbody>
                <tr v-for="pan in vendorForm.vendorpan" :key="pan.id">
                  ...
                  <td><span :class="['pill', pan.partner ? 'pill--indigo' : 'pill--slate']">{{ pan.partner ? 'Yes' :
                    'No'
                  }}</span></td>
                </tr>
              </tbody>
              ...
              <div class="modal-box">
                  ...
                  <div class="modal-grid">
                    ...
                    <div class="modal-field modal-field--full">
                      ...
                      <select v-model="lsForm.interfaceId" :disabled="!lsForm.serviceId">
                        <option value="">{{ lsForm.serviceId ? 'Select an interface' : 'Select a service first' }}
                        </option>
                ...
                <div class="modal-box__footer">
                  ...
                  <button class="btn-primary" :disabled="lsSaving" @click="saveLs">
                    <svg v-if="lsSaving" class="spin-icon" width="14" height="14" viewBox="0 0 24 24" fill="none"
                      stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
      ...
      <section v-show="activeTab === 'commissions'" class="tab-section">
        ...
        <div class="card" v-if="vendorForm.aggregator">
          ...
          <div class="info-grid info-grid--4">
            <div class="info-item"><label>Name</label>
              <p>{{ vendorForm.aggregator.name }}</p>
            </div>
            <div class="info-item"><label>Email</label>
              <p>{{ vendorForm.aggregator.email }}</p>
            </div>
            <div class="info-item"><label>Mobile</label>
              <p>{{ vendorForm.aggregator.mobile_no }}</p>
            </div>
            <div class="info-item"><label>Code</label>
              <p class="font-mono">{{ vendorForm.aggregator.code }}</p>
            </div>
            <div class="info-item"><label>Default Rate</label>
              <p>{{ vendorForm.aggregator.rate }}% ({{ vendorForm.aggregator.rateType }})</p>
            </div>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div v-if="vendorForm.commissionconfig?.length">
            <div v-for="cfg in vendorForm.commissionconfig" :key="cfg.id" class="cc-row">
              <!-- Config header row -->
              <div class="cc-row__head" @click="toggleCfgExpand(cfg.id)">
                ...
                <div class="cc-row__range font-mono">
                  ₹{{ Number(cfg.minAmount).toLocaleString('en-IN') }} – ₹{{
                    Number(cfg.maxAmount).toLocaleString('en-IN') }}
                </div>
      ...
      <section v-show="activeTab === 'cards'" class="tab-section">
        ...
        <div class="card">
          <div class="card__head">
            ...
            <span class="ml-2">
              <span class="pill pill--indigo pill--sm">{{ vendorCards.length }} total</span>
            </span>
            <div class="search-box cards-search ml-auto">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                stroke-linecap="round" stroke-linejoin="round">
                <circle cx="11" cy="11" r="8" />
                <line x1="21" y1="21" x2="16.65" y2="16.65" />
              </svg>
              <input v-model="cardSearch" placeholder="Search cards…" class="search-input" />
            </div>
            <button class="btn-primary" style="margin-left:10px" @click="openAddCard">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"
                stroke-linecap="round" stroke-linejoin="round">
                <line x1="12" y1="5" x2="12" y2="19" />
                <line x1="5" y1="12" x2="19" y2="12" />
          ...
          <!-- Stats ribbon -->
          <div class="cards-ribbon" v-if="vendorCards.length">
            <div v-for="s in CARD_STATUSES" :key="s" class="ribbon-pill">
              <span :class="['pill pill--sm', cardStatusPill(s)]">{{ s }}</span>
              <span class="ribbon-count">{{vendorCards.filter(c => c.status === s).length}}</span>
            </div>
          </div>

          <!-- Table -->
          <div class="table-scroll-wrap" v-if="filteredCards.length">
            <table class="data-table">
              <thead>
                <tr>
                  <th>Holder Name</th>
              ...
              <tbody>
                <tr v-for="c in filteredCards" :key="c.id">
                  <!-- Holder Name -->
                  <td>
                    <div class="card-holder-cell">
                      <div class="card-avatar" :style="{ background: avatarBg(c.cardHolderName) }">
                        {{ (c.cardHolderName || 'C').charAt(0).toUpperCase() }}
                      </div>
                      <span class="font-bold text-xs">{{ c.cardHolderName }}</span>
                    </div>
                  </td>

                  <!-- Masked PAN -->
                  <td>
                    <span class="font-mono text-xs card-pan">{{ c.maskedPan }}</span>
                  </td>

                  <!-- Type -->
                  <td>
                    <span :class="['pill pill--sm', cardTypePill(c.type)]">{{ c.type }}</span>
                  </td>

                  <!-- Network -->
                  <td>
                    <span :class="['pill pill--sm', cardNetworkPill(c.network)]">{{ c.network }}</span>
                  </td>

                  <!-- Status — inline select for quick update -->
                  <td>
                    <div class="status-select-wrap">
                      <select :value="c.status" @change="updateCardStatus(c, $event.target.value)"
                        :class="['status-select', `status-select--${c.status.toLowerCase()}`]">
                  ...
                  <td class="font-mono text-xs">
                    <span v-if="c.expiryMonth && c.expiryYear">
                  ...
                  <!-- Issuer -->
                  <td>
                    <span class="pill pill--sm pill--slate">{{ c.issuerBank }}</span>
                  </td>

                  <!-- Limits -->
                  <td>
                    <div class="limits-cell">
                      <span v-if="c.dailyLimit" class="limit-badge">D: ₹{{
                        Number(c.dailyLimit).toLocaleString('en-IN') }}</span>
                      <span v-if="c.perTxnLimit" class="limit-badge">T: ₹{{
                        Number(c.perTxnLimit).toLocaleString('en-IN') }}</span>
                      <span v-if="c.availableLimit" class="limit-badge limit-badge--avail">A: ₹{{
                        Number(c.availableLimit).toLocaleString('en-IN') }}</span>
                      <span v-if="!c.dailyLimit && !c.perTxnLimit && !c.availableLimit"
                        style="color:#94a3b8;font-size:11px">—</span>
                    </div>
                  </td>

                  <!-- KYC -->
                  <td>
                    <span :class="['flag', c.kycVerified ? 'flag--on' : 'flag--off']">
                  ...
                  <td>
                    <div class="action-btns">
                      <button class="icon-btn icon-btn--edit" @click="openEditCard(c)" title="Edit card">
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                          stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
                          <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                          <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
                      ...
                      <button class="icon-btn icon-btn--delete" @click="deleteCard(c.id)" title="Delete card">
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                          stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
                          <polyline points="3 6 5 6 21 6" />
                          <path d="M19 6l-1 14H6L5 6" />
          ...
          <div class="empty-state" v-else>
            ...
            <p>{{ cardSearch ? 'No cards match your search' : 'No cards found for this vendor' }}</p>
            <button class="btn-primary" style="margin-top:10px" @click="openAddCard">Add First Card</button>
          </div>
        </div>
      </section>
      ...
      <Teleport to="body">
        <Transition name="modal-fade">
          <div v-if="cardModal.open" class="modal-overlay" @click.self="closeCardModal">
            <div class="modal-box modal-box--wide">

              <div class="modal-box__header">
                <h3 class="modal-box__title">
                  {{ cardModal.mode === 'add' ? 'Add New Card' : 'Edit Card' }}
                </h3>
              ...
              <div class="modal-box__body">
                ...
                <!-- Row 1: Core identity -->
                <p class="cfg-section-label">Card Details</p>
                <div class="modal-grid">
                  <div class="modal-field modal-field--full">
                    <label>Card Holder Name <span class="req">*</span></label>
                    <input v-model="cardForm.cardHolderName" placeholder="e.g. John Doe" />
                  </div>
                  <div class="modal-field modal-field--full">
                    <label>Masked PAN</label>
                    <input v-model="cardForm.maskedPan" placeholder="e.g. 524164XXXXXX4587" class="font-mono" />
                  </div>
                  <div class="modal-field">
                  ...
                  <div class="modal-field">
                    <label>Network <span class="req">*</span></label>
                  ...
                  <div class="modal-field">
                    ...
                    <select v-model="cardForm.issuerBank">
                ...
                <!-- Row 2: Expiry -->
                <p class="cfg-section-label" style="margin-top:20px">Expiry</p>
                <div class="modal-grid">
                  <div class="modal-field">
                    <label>Expiry Month (1–12, 0 = not set)</label>
                    <input type="number" v-model.number="cardForm.expiryMonth" min="0" max="12" />
                  </div>
                  <div class="modal-field">
                    <label>Expiry Year (YYYY, 0 = not set)</label>
                    <input type="number" v-model.number="cardForm.expiryYear" min="0" max="2099" />
                  </div>
                </div>

                <!-- Row 3: Limits -->
                <p class="cfg-section-label" style="margin-top:20px">Limits (₹ — leave blank to skip)</p>
                <div class="modal-grid">
                  <div class="modal-field">
                    <label>Daily Limit</label>
                    <input type="number" v-model="cardForm.dailyLimit" min="0" placeholder="e.g. 100000" />
                  </div>
                  <div class="modal-field">
                    <label>Per Transaction Limit</label>
                    <input type="number" v-model="cardForm.perTxnLimit" min="0" placeholder="e.g. 50000" />
                  </div>
                  <div class="modal-field">
                    <label>Available Limit</label>
                    <input type="number" v-model="cardForm.availableLimit" min="0" placeholder="e.g. 200000" />
                  </div>
                </div>

                <!-- Row 4: Toggles -->
                <p class="cfg-section-label" style="margin-top:20px">Settings</p>
                <div class="modal-grid">
                  <div class="modal-field modal-field--full">
                    <label class="toggle-label">
                      <span>International Transactions Enabled</span>
                      <div class="toggle-wrap">
                        <input type="checkbox" v-model="cardForm.internationalTxn" class="toggle-input"
                          id="cardIntlToggle" />
                        <label for="cardIntlToggle" class="toggle-track"></label>
                      </div>
                    </label>
                  </div>
                  <div class="modal-field modal-field--full">
                    <label class="toggle-label">
                      ...
                      <div class="toggle-wrap">
                        <input type="checkbox" v-model="cardForm.contactless" class="toggle-input"
                          id="cardContactlessToggle" />
                        <label for="cardContactlessToggle" class="toggle-track"></label>
                      </div>
                    </label>
                  </div>
                  <div class="modal-field modal-field--full">
                    <label class="toggle-label">
                      ...
                      <div class="toggle-wrap">
                        <input type="checkbox" v-model="cardForm.kycVerified" class="toggle-input" id="cardKycToggle" />
                        <label for="cardKycToggle" class="toggle-track"></label>
                      </div>
                    </label>
                  </div>
                </div>
              ...
              <div class="modal-box__footer">
                <button class="btn-cancel" @click="closeCardModal">Cancel</button>
                <button class="btn-primary" :disabled="cardSaving" @click="saveCard">
                  <svg v-if="cardSaving" class="spin-icon" width="14" height="14" viewBox="0 0 24 24" fill="none"
                    stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M21 12a9 9 0 1 1-6.219-8.56" />
                  </svg>
                  {{ cardSaving ? 'Saving…' : cardModal.mode === 'add' ? 'Create Card' : 'Save Changes' }}
                </button>
              </div>
      ...
      <section v-show="activeTab === 'merchants'" class="tab-section">
        ...
        <div class="toolbar-row">
          ...
          <div class="filter-pills">
            <button :class="['fpill', merchantFilter === 'ALL' && 'fpill--active']"
              @click="merchantFilter = 'ALL'">All</button>
            <button :class="['fpill', merchantFilter === 'APPROVED' && 'fpill--active']"
              @click="merchantFilter = 'APPROVED'">Approved</button>
            <button :class="['fpill', merchantFilter === 'PENDING' && 'fpill--active']"
              @click="merchantFilter = 'PENDING'">Pending</button>
          </div>
        ...
        <div class="merchant-grid" v-if="merchantView === 'card' && filteredMerchants.length">
          <div v-for="m in filteredMerchants" :key="m.id" class="merchant-card">
            <div class="merchant-card__top" @click="toggleExpand(m.id)" style="cursor:pointer">
              ...
              <div class="mc-badges">
                <span :class="['pill pill--sm', m.status ? 'pill--emerald' : 'pill--red']">{{ m.status ? 'Active' :
                  'Inactive'
                }}</span>
            ...
            <div class="mc-methods" v-if="m.paymethods?.length">
              ...
              <span v-for="pm in m.paymethods" :key="pm.paymentMethod" class="pill pill--sm pill--sky">{{
                pm.paymentMethod }}
                · {{ pm.provider }}</span>
            </div>
            <div class="mc-kyc-row" v-if="m.merchantKyc">
              <span class="mc-methods__label">KYC</span>
              <span :class="['pill pill--sm', kycPillClass(m.merchantKyc.globalStatus)]">{{ m.merchantKyc.globalStatus
              }}</span>
            ...
            <div v-if="expandedMerchant === m.id" class="mc-expanded">
              <div class="mc-exp-grid">
                ...
                <div class="mc-exp-block" v-if="m.merchantinfo">
                  ...
                  <p class="mc-exp-val" v-if="m.merchantinfo.annual_turn_over">Turnover: {{
                    m.merchantinfo.annual_turn_over }}
                  </p>
                </div>
                ...
                <div class="mc-exp-block" v-if="m.merchantpan?.length">
                  ...
                  <div v-for="pan in m.merchantpan" :key="pan.id" class="mc-exp-pan-row">
                    ...
                    <span :class="['pill pill--sm', pan.partner ? 'pill--indigo' : 'pill--slate']">{{ pan.partner ?
                      'Partner'
                      : 'Self' }}</span>
                  </div>
                </div>
                <div class="mc-exp-block" v-if="m.wallet">
                  ...
                  <p class="mc-exp-val font-bold">₹ {{ Number(m.wallet.balance).toLocaleString('en-IN', {
                    minimumFractionDigits: 2
                  }) }}</p>
                  <span :class="['pill pill--sm', m.wallet.status ? 'pill--emerald' : 'pill--red']">{{ m.wallet.status ?
                ...
                <table class="data-table data-table--compact kyc-svc-desktop">
                  ...
                  <tbody>
                    <tr v-for="svc in m.merchantKyc.services" :key="svc.id">
                      ...
                      <td><span :class="['pill pill--sm', kycPillClass(svc.aadhaarStatus)]">{{ svc.aadhaarStatus
                      }}</span>
                      </td>
                      <td><span :class="['pill pill--sm', kycPillClass(svc.bankStatus)]">{{ svc.bankStatus }}</span>
                      </td>
                      <td><span :class="['pill pill--sm', kycPillClass(svc.gstStatus)]">{{ svc.gstStatus }}</span></td>
                      <td><span :class="['pill pill--sm', kycPillClass(svc.storeImgStatus)]">{{ svc.storeImgStatus
                      }}</span>
                      </td>
                      <td><span :class="['flag', svc.otpStatus ? 'flag--on' : 'flag--off']">{{ svc.otpStatus ? 'Done' :
                        'No'
                      }}</span></td>
                ...
                <div class="kyc-svc-mobile">
                  <div v-for="svc in m.merchantKyc.services" :key="svc.id" class="kyc-mob-card">
                    ...
                    <div class="kyc-mob-card__body">
                      <div class="kyc-mob-row"><span>PAN</span><span
                          :class="['pill pill--sm', kycPillClass(svc.panStatus)]">{{ svc.panStatus }}</span></div>
                      <div class="kyc-mob-row"><span>Aadhaar</span><span
                          :class="['pill pill--sm', kycPillClass(svc.aadhaarStatus)]">{{ svc.aadhaarStatus }}</span>
                      </div>
                      <div class="kyc-mob-row"><span>Bank</span><span
                          :class="['pill pill--sm', kycPillClass(svc.bankStatus)]">{{ svc.bankStatus }}</span></div>
                      <div class="kyc-mob-row"><span>GST</span><span
                          :class="['pill pill--sm', kycPillClass(svc.gstStatus)]">{{ svc.gstStatus }}</span></div>
                      <div class="kyc-mob-row"><span>Store Img</span><span
                          :class="['pill pill--sm', kycPillClass(svc.storeImgStatus)]">{{ svc.storeImgStatus }}</span>
                      </div>
                      <div class="kyc-mob-row"><span>OTP</span><span
                          :class="['flag', svc.otpStatus ? 'flag--on' : 'flag--off']">{{ svc.otpStatus ? 'Done' : 'No'
                          }}</span></div>
                      <div class="kyc-mob-row"><span>Biometric</span><span
                          :class="['flag', svc.biometricRequired ? 'flag--on' : 'flag--off']">{{ svc.biometricRequired ?
                            'Required' : 'No' }}</span></div>
                      <div class="kyc-mob-row"><span>Physical Verify</span><span
                          :class="['flag', svc.physicalVerificationRequired ? 'flag--on' : 'flag--off']">{{
                            svc.physicalVerificationRequired ? 'Yes' : 'No' }}</span></div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <table class="data-table">
              ...
              <tbody>
                <tr v-for="m in filteredMerchants" :key="m.id">
                  ...
                  <td><span :class="['pill pill--sm', m.status ? 'pill--emerald' : 'pill--red']">{{ m.status ? 'Active'
                    :
                    'Inactive' }}</span></td>
                  <td><span class="pill pill--sm pill--amber">{{ m.mstatus }}</span></td>
                  <td><span :class="['pill pill--sm', m.riskflag > 0 ? 'pill--red' : 'pill--emerald']">{{ m.riskflag > 0
                    ?
                    'Flagged' : 'Clear' }}</span></td>
        ...
        <div class="terminal-grid" v-if="vendorForm.terminals?.length">
          <div class="terminal-card" v-for="t in vendorForm.terminals" :key="t.id">
            <div class="terminal-card__top">
              ...
              <span :class="['pill pill--sm ml-auto', t.status ? 'pill--emerald' : 'pill--red']">{{ t.status ? 'Active'
                :
                'Inactive' }}</span>
            </div>
            <div class="info-grid info-grid--3">
              <div class="info-item"><label>MID</label>
                <p class="font-mono text-xs">{{ t.mid || '—' }}</p>
              </div>
              <div class="info-item"><label>Interface</label>
                <p>{{ t.interface }}</p>
              </div>
              <div class="info-item"><label>Serial No</label>
                <p class="font-mono text-xs">{{ t.serialNumber || '—' }}</p>
              </div>
              <div class="info-item"><label>VPA</label>
                <p class="font-mono text-xs">{{ t.vpastring?.vpa || '—' }}</p>
              </div>
              <div class="info-item"><label>Soundbox</label>
                <p><span :class="['flag', t.soundbox ? 'flag--on' : 'flag--off']">{{ t.soundbox ? 'Yes' : 'No' }}</span>
                </p>
              </div>
              <div class="info-item"><label>Risk Flag</label>
                <p><span :class="['pill pill--sm', t.riskflag > 0 ? 'pill--red' : 'pill--emerald']">{{ t.riskflag
                }}</span>
                </p>
              </div>
            </div>
        ...
        <div class="docs-grid" v-if="vendorForm.documents?.length">
          <div class="doc-card" v-for="doc in vendorForm.documents" :key="doc.id" @click="openDoc(doc)">
            <div class="doc-card__top">
              <div class="doc-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"
    ...
    <footer class="mh-footer">
      <div class="mh-footer__status">
        <div class="pulse-dot"></div><span>Live System Status: Optimal</span>
      </div>
      <div class="mh-footer__links"><a href="#">Support Docs</a><a href="#">Compliance</a><a href="#">API Reference</a>
      </div>
    </footer>
          ...
          <div class="dialog">
            <div class="dialog__header">
              ...
              <div class="dialog__hdr-right">
                <span :class="['pill pill--sm', docStatusPill(selectedDoc?.doc_status)]">{{ selectedDoc?.doc_status
                }}</span>
            ...
            <div class="dialog__body" v-if="selectedDoc">
              <div class="info-grid info-grid--3">
                <div class="info-item"><label>Document No</label>
                  <p class="font-mono">{{ selectedDoc.doc_number || '—' }}</p>
                </div>
                <div class="info-item"><label>Verified By</label>
                  <p>{{ selectedDoc.doc_verified_by || '—' }}</p>
                </div>
                <div class="info-item"><label>Result</label>
                  <p><span
                      :class="['pill pill--sm', selectedDoc.doc_verified_result ? 'pill--emerald' : 'pill--amber']">{{
                        selectedDoc.doc_verified_result ? 'Verified' : 'Pending' }}</span></p>
                </div>
                <div class="info-item"><label>Remark</label>
...
const { getVendorCards, createVendorCard, updateVendorCard, deleteVendorCard } = useVendorCardApi()
...
// ── Cards state ──────────────────────────────────────────────────
const vendorCards = ref([])
const cardSearch = ref('')
const cardSaving = ref(false)

const cardModal = reactive({ open: false, mode: 'add' })  // mode: 'add' | 'edit'

const defaultCardForm = () => ({
  id: null,
  cardHolderName: '',
  maskedPan: '000000XXXXXX0000',
  type: 'CDM',
  network: 'VISA',
  status: 'PENDING',
  expiryMonth: 0,
...
const cardForm = reactive(defaultCardForm())

// ── Filtered cards ───────────────────────────────────────────────
const filteredCards = computed(() => {
  const q = cardSearch.value.toLowerCase()
  return q
    ? vendorCards.value.filter(c =>
      (c.cardHolderName || '').toLowerCase().includes(q) ||
      (c.maskedPan || '').toLowerCase().includes(q) ||
...
const fetchVendorCards = async () => {
  const res = await getVendorCards(props.vendorId)
  vendorCards.value = res?.data ?? []
}

// ── Modal helpers ────────────────────────────────────────────────
const openAddCard = () => {
  Object.assign(cardForm, defaultCardForm())
  cardModal.mode = 'add'
  cardModal.open = true
}
...
const openEditCard = (card) => {
  Object.assign(cardForm, {
    ...
    internationalTxn: card.internationalTxn,
    contactless: card.contactless,
    kycVerified: card.kycVerified,
  })
  cardModal.mode = 'edit'
  cardModal.open = true
}

const closeCardModal = () => { cardModal.open = false }

// ── Save (create or update) ───────────────────────────────────────
const saveCard = async () => {
  if (!cardForm.cardHolderName.trim()) {
    showSnack('Card holder name is required', 'error'); return
  }
  if (!cardForm.type || !cardForm.network) {
    showSnack('Type and Network are required', 'error'); return
  }
  ...
  try {
    const payload = {
      ...
      issuerBank: cardForm.issuerBank,
      internationalTxn: cardForm.internationalTxn,
      contactless: cardForm.contactless,
      kycVerified: cardForm.kycVerified,
      ...(cardForm.dailyLimit !== '' && { dailyLimit: Number(cardForm.dailyLimit) }),
      ...(cardForm.availableLimit !== '' && { availableLimit: Number(cardForm.availableLimit) }),
      ...(cardForm.perTxnLimit !== '' && { perTxnLimit: Number(cardForm.perTxnLimit) }),
    }

    const res = cardModal.mode === 'add'
      ? await createVendorCard(props.vendorId, payload)
      : await updateVendorCard(props.vendorId, cardForm.id, payload)

    const ok = res?.statusCode === '00'
    showSnack(ok ? res.message || 'Saved' : res?.message || 'Failed', ok ? 'success' : 'error')
    if (ok) { closeCardModal(); fetchVendorCards() }
  } catch (e) {
    showSnack('Failed to save card', 'error')
  } finally {
...
const updateCardStatus = async (card, newStatus) => {
  const res = await updateVendorCard(props.vendorId, card.id, { status: newStatus })
  const ok = res?.statusCode === '00'
  showSnack(ok ? `Status → ${newStatus}` : res?.message || 'Failed', ok ? 'success' : 'error')
  if (ok) fetchVendorCards()
}

// ── Delete ────────────────────────────────────────────────────────
const deleteCard = async (cardId) => {
  if (!confirm('Delete this card? This action cannot be undone.')) return
  const res = await deleteVendorCard(props.vendorId, cardId)
  const ok = res?.statusCode === '00'
  showSnack(ok ? 'Card deleted' : res?.message || 'Delete failed', ok ? 'success' : 'error')
  if (ok) fetchVendorCards()
}
...
const cardTypePill = (t) => {
  ...
  return map[t] ?? 'pill--slate'
}

const cardNetworkPill = (n) => {
  const map = {
    VISA: 'pill--indigo',
    MASTERCARD: 'pill--red',
    RUPAY: 'pill--emerald',
...
.info-grid--4 .info-item:nth-child(4n) {
  border-right: none;
}
...
@media(max-width:1024px) {
  ...
  .info-grid--3 .info-item:nth-child(2n) {
    border-right: none;
  }
...
.status-select--active    { background-color: #d1fae5; color: #065f46; border-color: #86efac; }
.status-select--blocked   { background-color: #fee2e2; color: #991b1b; border-color: #fca5a5; }
.status-select--hotlisted { background-color: #fee2e2; color: #991b1b; border-color: #fca5a5; }
.status-select--expired   { background-color: #f1f5f9; color: #475569; border-color: #cbd5e1; }
.status-select--inactive  { background-color: #fef3c7; color: #92400e; border-color: #fcd34d; }
.status-select--availabil { background-color: #e0f2fe; color: #0369a1; border-color: #7dd3fc; }
.status-select--pending   { background-color: #fef3c7; color: #92400e; border-color: #fcd34d; }
...
/* ── Cards search box (header-inline) ── */
.cards-search {
  max-width: 200px;
  height: 32px;
  font-size: 12px;
...
/* ── Mobile adjustments ── */
@media (max-width: 640px) {
  .cards-ribbon { gap: 6px; padding: 8px 14px; }
