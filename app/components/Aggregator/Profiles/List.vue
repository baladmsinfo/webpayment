<template>
  <div class="page-root">
    ...
    <div class="page-header">
      <div>
        <h1 class="page-title">List of Profiles</h1>
        <p class="page-sub">All profiles registered under your aggregator account</p>
      </div>
            ...
            >
              ...
              <td>
                <div class="merchant-cell">
                  <div class="merchant-avatar" :style="{ background: avatarColor(item.name) }">
                    {{ initials(item.name) }}
                  </div>
                  <div>
                    <p class="merchant-name">{{ item.name }}</p>
                    <p class="merchant-mid th-hide-lg">{{ item.code }}</p>
                  </div>
                </div>
              </td>
              ...
              <!-- Code -->
              <td class="th-hide-sm">
                <span class="mid-val">{{ item.code }}</span>
              </td>
              ...
              <td class="th-hide-md">
                <div class="risk-cell">
                  <span class="risk-dot" :class="!item.riskflag ? 'risk-ok' : 'risk-bad'"></span>
                  <span class="risk-label" :class="!item.riskflag ? 'risk-ok-text' : 'risk-bad-text'">
                    {{ !item.riskflag ? 'Low' : 'High' }}
                  </span>
                </div>
              </td>
              ...
              <!-- Action -->
              <td @click.stop>
                <button class="btn-view" @click="goToProfile(item)" title="View Profile">
              ...
              <td colspan="8" class="empty-td">
                <div class="empty-state">
                  <div class="empty-icon">
                    <span class="mdi mdi-account-off-outline"></span>
                  </div>
...
const { getWalletProfiles } = useWalletProfileApi();
...
const mstatusClass = (s: string) => ({
  ...
  'chip-red':    ['SUSPENDED','BLOCKED'].includes(s),
  'chip-grey':   !['PENDING','APPROVED','SUSPENDED','BLOCKED'].includes(s),
});
...
async function loadProfiles() {
  loading.value = true;
  try { await getWalletProfiles({ page: page.value, limit: limit.value }); }
  finally { loading.value = false; }
}
async function refresh() { await loadProfiles(); }
function onPageChange(p: number)  { page.value = p;     loadProfiles(); }
function onLimitChange(l: number) { limit.value = l; page.value = 1; loadProfiles(); }

/* ── Row click ── */
const goToProfile = (item: any) => router.push(`/aggregator/profiles/view/${item.id}`);
