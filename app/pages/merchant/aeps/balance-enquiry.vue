<template>
  <MerchantDmtShell title="Balance Enquiry" :subtitle="subtitle" back-to="/merchant/aeps" :show-back="step !== 'result'">
    ...
    <div class="btn-row mt-4" v-if="step === 'result'">
      <v-btn variant="outlined" :color="BX.primary" class="flex-1 action-btn" @click="tryAgain">
        <v-icon start size="17">mdi-reload</v-icon>New Enquiry
      </v-btn>
      <v-btn variant="flat" :color="BX.primary" class="flex-1 action-btn" @click="goHome">Home</v-btn>
    </div>

    <MerchantDmtProcessingOverlay :show="aepsStore.isProcessing" text="Checking account balance…" />
  </MerchantDmtShell>
</template>
...
const subtitle = computed(() => ({
  ...
  result: "Step 3 of 3 · Balance result",
}[step.value]));
...
const onCaptured = async ({ pidData, coords }) => {
  ...
  const res = await aepsStore.performBalanceEnquiry();
  if (!res.status) { notify(res.message || "A transaction is already in progress", "error"); return; }
  step.value = "result";
  if (res.status === "SUCCESS") { notify("Balance retrieved successfully", "success"); walletStripRef.value?.fetchBalance(); }
  else if (res.status === "FAILED") notify(aepsStore.currentResult?.reason || "Balance enquiry failed", "error");
