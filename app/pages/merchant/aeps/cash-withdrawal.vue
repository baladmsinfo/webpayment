<template>
  <MerchantDmtShell title="Cash Withdrawal" :subtitle="subtitle" back-to="/merchant/aeps" :show-back="step !== 'result'">
    <MerchantDmtWalletStrip ref="walletStripRef" class="mb-4" />

    <MerchantAepsCustomerBankForm
      v-if="step === 'form'"
      with-amount
      amount-label="Withdrawal Amount"
    ...
    <div class="btn-row mt-4" v-if="step === 'result'">
      <v-btn variant="outlined" :color="BX.primary" class="flex-1 action-btn" @click="tryAgain">
        <v-icon start size="17">mdi-reload</v-icon>New Withdrawal
      </v-btn>
      <v-btn variant="flat" :color="BX.primary" class="flex-1 action-btn" @click="goHome">Home</v-btn>
    </div>

    <MerchantDmtProcessingOverlay :show="aepsStore.isProcessing" text="Processing cash withdrawal…" />
  </MerchantDmtShell>
</template>
...
// useMerchantBalanceApi() isn't a singleton — each call gets independent state — so we
// read/refresh through the WalletStrip's own instance (via template ref) rather than
// maintaining a second, disconnected balance fetch that the strip on screen never sees.
...
const subtitle = computed(() => ({
  ...
  result: "Step 3 of 3 · Transaction result",
}[step.value]));
...
const onCaptured = async ({ pidData, coords }) => {
  ...
  const res = await aepsStore.performWithdrawal();
  if (!res.status) { notify(res.message || "A transaction is already in progress", "error"); return; }
  step.value = "result";
  if (res.status === "SUCCESS") { notify("Cash withdrawal successful", "success"); walletStripRef.value?.fetchBalance(); }
  else if (res.status === "FAILED") notify(aepsStore.currentResult?.reason || "Withdrawal failed", "error");
