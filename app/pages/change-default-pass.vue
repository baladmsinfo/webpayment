  ...
  <div class="login-root">
    ...
    <div class="login-left">
      <div class="left-inner">
        ...
        <!-- Copy -->
        <div class="left-copy">
          <h2 class="left-headline">Welcome aboard —<br />make it yours.</h2>
          <p class="left-sub">
            You're using a system-generated password. Set a personal password now
            to keep your account secure from day one.
          </p>
        </div>
    ...
    <div class="login-right">
      <div class="login-form-wrap">
        ...
        <div class="first-login-badge">
          <span class="mdi mdi-shield-alert-outline badge-icon"></span>
          <span>Update your auto-generated password to ensure security and protection</span>
        </div>
        ...
        <div class="form-icon-wrap">
          <div class="form-icon-circle" :class="{ 'form-icon-success': succeeded }">
            <span
              class="mdi"
              :class="succeeded ? 'mdi-check-bold' : 'mdi-lock-open-variant-outline'"
            ></span>
          </div>
        ...
        <div class="form-heading">
          <h1 class="form-title">
            {{ succeeded ? 'Password Updated!' : 'Set Your Password' }}
          </h1>
          <p class="form-sub">
            {{
              succeeded
                ? 'Your new password is active. Taking you to your dashboard…'
                : 'Replace your temporary password with something only you know.'
            }}
          </p>
        </div>
        ...
        <div v-if="!succeeded" class="form-fields">
          ...
          <div class="field-group">
            ...
            <div
              class="input-wrap"
              :class="{ 'input-focused': focuses.new, 'input-error': errors.new }"
            >
              ...
              <button class="pw-toggle" @click="show.new = !show.new" tabindex="-1">
                <span
                  class="mdi"
                  :class="show.new ? 'mdi-eye-off-outline' : 'mdi-eye-outline'"
                ></span>
              </button>
            </div>
          ...
          <div class="field-group">
            ...
            >
              ...
              <button class="pw-toggle" @click="show.confirm = !show.confirm" tabindex="-1">
                <span
                  class="mdi"
                  :class="show.confirm ? 'mdi-eye-off-outline' : 'mdi-eye-outline'"
                ></span>
              </button>
            </div>
          ...
          <!-- Skip -->
          <button class="btn-skip" :disabled="loading" @click="onSkip">
...
const canSubmit = computed(() =>
  newPassword.value.length >= 6 && confirmPassword.value === newPassword.value,
);
...
function goToDashboard() {
  const dest = ROLE_ROUTES[props.role?.toLowerCase()] ?? '/';
  router.push(dest);
}
...
function startRedirect() {
  ...
  const t = setInterval(() => {
    ...
    if (elapsed >= total) { clearInterval(t); goToDashboard(); }
  }, interval);
}
...
async function onSubmit() {
  ...
  try {
    const res = await changeDefaultPassword({ newPassword: newPassword.value });

    if (res?.statusCode === '00' || res?.data?.statusCode === '00') {
      succeeded.value = true;
      startRedirect();
    } else {
      errorMessage.value = res?.message || res?.data?.message || 'Failed to update password.';
    }
  } catch (e) {
...
.badge-icon { font-size: 15px; color: #f59e0b; }
...
.btn-skip:hover:not(:disabled) {
  border-color: #94a3b8; color: #475569; background: #f8fafc;
}
.btn-skip:disabled { opacity: .4; cursor: not-allowed; }
.btn-skip .mdi { font-size: 15px; }
